...
class Thumbnail {
  ...
  function Thumbnail($fileName) {
	...
	$this->errmsg = 'Unknown file format' . $this->fileName;
	$this->error = true;
      }
    }
  ...
  function show($quality=100,$name = '') {
    ...
    case 'GIF':
      if($name != '') {
	return ImageGif($this->newImage,$name);
      }
      else {
    ...
    case 'JPG':
      if($name != '') {
	return ImageJpeg($this->newImage,$name,$quality);
      }
      else {
	header('Content-type: image/jpeg');
	return ImageJpeg($this->newImage,'',$quality);
      }
      break;
    case 'PNG':
      if($name != '') {
	return ImagePng($this->newImage,$name);
      }
      else {
	header('Content-type: image/png');
