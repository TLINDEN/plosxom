<?
 //
 //   $Id: standard.php 156 2008-01-28 00:12:14Z palidhar $
 //
...
function hook_storage_fetchall($unlimited = false) {
  /*
  ...
  */
  ...
  function getfile($datadir, $file, $dir="", $read=true, $paragraph=true) {
    //
    ...
    if(is_readable($filename) and ereg('\.txt$', $file)) {
      ...
      if($read) {
         ...
         $entry["text"]  = paragraph(implode('', $lines), $paragraph);
...
function paragraph(&$text, $paragraph=true) {
  if($paragraph) {
    $paragraph = " class='blogparagraph'";
  }
  $text = preg_replace("/(\r\n\r\n|\n\n)/", "</p><p$paragraph>", $text);
